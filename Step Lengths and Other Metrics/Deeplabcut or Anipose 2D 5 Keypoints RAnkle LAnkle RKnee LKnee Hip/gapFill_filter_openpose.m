function gapFill_filter_openpose(output_name)
clearvars -except output_name
file = sprintf('%s%s',output_name,'_openpose.mat');
cd = pwd;
%%
noLandmarks = 5; % BODY_25 model
...
markers = ['RAnkle', 'LAnkle', 'RKnee', 'LKnee', 'Hip'];
axes = {'X','Y'};

load(fullfile(cd,file),'data_openpose','videoInfo','frameInfo')
    
noFiles = length(data_openpose.time);
    
%% interpolate over gaps 
data_openpose.gapFill_data = nan(size(data_openpose.corrected_data));
frameInfo.isGapFilled = false(noFiles,noLandmarks); % logical matrix showing which frames are gap filled

for j = 1:noLandmarks
    for k = 1:2 
        if sum(~isnan(data_openpose.corrected_data(:,j,k))) > 1 % check that number of tracked data points exceed 1, otherwise "interp1gap" cannot gap fill
            data_openpose.gapFill_data(:,j,k) = interp1gap(data_openpose.time,data_openpose.corrected_data(:,j,k),data_openpose.time,0.120); % gap fill over gaps spanning less than 120 ms (ie. only gap filled over gaps of 2 or 1 frames at 25 Hz)
        end
        nan_data = isnan(data_openpose.corrected_data(:,j,1)); %check which frames are nan
        frameInfo.isGapFilled(nan_data,j) = ~isnan(data_openpose.gapFill_data(nan_data,j,1)); % check if nan frames have been interpolated
    end
end; clearvars j k
%% low-pass filter interpolated data
%% apply zero-lag low-pass Butterworth filtering with cut-off frequency of X Hz
data_openpose.filt_data = nan(size(data_openpose.gapFill_data));
...
for j = 1:noLandmarks
for k = 1:2
data_openpose.filt_data(:,j,k) = nanfiltfilt(B,A,data_openpose.gapFill_data(:,j,k));
end
end; clearvars j k
%% save gap filled and filtered data to openpose file
save(fullfile(cd,file),'data_openpose','frameInfo','-append')
end
